...
// 				JHtmlBootstrap::loadCss(true, $document->direction);
	...
	public static function load_editor($editor, $id, $name, $html, $rows, $cols, $width=null, $height=null, $class=null, $style=null, $custom_tag = false){
		...
		if($editor == 'bbcode') {
			...
			CJFunctions::add_script(CJLIB_MEDIA_URI.'/sceditor/development/jquery.sceditor.bbcode.js', $custom_tag);
	...
	public static function parse_html($content, $process_content_plugins = false, $bbcode = true, $autolink = true){
		...
		if($bbcode){
			...
			require_once CJLIB_PATH.'/lib/nbbc/nbbc.php';
			$bbcode = new BBCode();
...
// 					$template->setSideWide( true );
	...
	public static function get_formatted_date($strdate) {
		...
		$future_date = $diff < 0 ? true : false;
		$suffix = $future_date ? JText::_('COM_CJLIB_DATE_SUFFIX_FROM_NOW') : JText::_('COM_CJLIB_DATE_SUFFIX_AGO');
		...
		if($dayDiff == 0) {
			...
			return $future_date ? JText::_('COM_CJLIB_TOMORROW') : JText::_('COM_CJLIB_YESTERDAY');
		} elseif($dayDiff < 7) {
